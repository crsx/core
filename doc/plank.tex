...
\item Rule \thetag{S-D-Data} expresses that a new data constructor can be introduced as belonging to
...
\item Conversely, \thetag{S-D-Fun} permits the introduction of ``defined'' function constructors:
  these can build a term of a generic sort (see Example~\ref{ex:pick}).

\item \thetag{S-D-Var} declares that a sort has syntactic variables, recorded in the sort
  environment. (This has consequences for the use of substitution in the sort below.)

\item \thetag{S-D-Rule} is the main sort checking of rules. Once the arbitrary sort has been
...
\item The rules in Figure~\ref{fig:termsortrules} gives the primary term rules per term context
  $TC$, which indicates the context of our meta-term fragment:
  %% 
  \begin{itemize}
...
\item The \thetag{SMP-*} rules handle patterns. The first rule \thetag{SMP-Fun} is the entry rule
  ...
  \thetag{SMP-Data,SMP-Meta,SMP-Var}, which capture the sort propagation as well as the special
  ...
  \item free variables only allowed in positions with a sort that includes a \kw{variable}
    declarations.
  \end{enumerate}

\item \thetag{SMC-*} rules handle contraction meta-terms. \TBD{More.}

\item \thetag{SMS-*} rules handle contraction substitution arguments. \TBD{More.}

\item \thetag{SP-*} rules handle ``pieces,'' \ie, scoped parameters to constructions. \TBD{More.}

\item \thetag{SA*}…\TBD{More.}
