...
\MS{About Figure~\ref{fig:syntax}
  ...
  \end{itemize}
} \KR{A: Fixed, yes but I suggest we keep'em, fixed, yes I like different braces but I did simplify
  decarations to not be grouped.}
...
\begin{definition}[\hax syntax]
  ...
  \KR{A: Yes, as now made explicit these are just the \emph{identifier} token.}
  %%
  \begin{itemize}
  ...
  \item A term of the form $c(P_1,…,P_n)$ ($n≥0$) is called a \emph{construction}; if furthermore
    ...
    \item Argument ``pieces'' $P$ of constructions take one of two forms: \emph{scopes}
      ...
      $T:T$, or an \emph{exclusion} $¬T$, or a \emph{catcher} ${:}M$.
  ...
  \MS{I'll take the above examples and continue with Associations of the form  $¬T$:}
   ...
   \KR{A: We talked about these in our call…any chance you can reduce, Maria?}
