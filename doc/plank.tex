...
\begin{definition}[\hax syntax]\label{def:syntax}
  %%
  The \hax syntax is shown in Figure~\ref{fig:syntax}, except we will also allow omission of empty
  "()" and "[]". The top level of a \hax script is a sequence of declarations $D^*$ and the grammar
  assumes that we have three categories of identifier tokens defined:
  %%
  \begin{itemize}
  ...
  meta-applications, and a \emph{pattern} if it is also a function construction. \KR{Yes, this needs
    to say that otherwise only data constructors are there.} \CK{This is somehwat tricky to work
    with recursively, for it makes $Z[x]$ not a pattern fragment if $Z$ is a meta-variable, giving
    problems with induction for a term like $f(x.g(Z[x]))$.}  \KR{I reintroduced checks for this in
    the sort rules (Definition~\ref{def:sort}). If you agree, we can clean up here.}
\end{definition}
...
\KR{I have no problem with switching the argument brackets to round, if you all prefer that.  If we
  want to be conventional then even using angles $⟨\,⟩$ for sort arguments as is common (Java, C++;
  Scala uses []).  Here are some samples of that:}
\begin{hacs}[numbers=right,texcl]
  List⟨α⟩ scheme Map([β]α, List⟨β⟩);
  List⟨α⟩ rule Map([x]#(x), Nil) →  Nil;
  List⟨Int⟩ rule Map([x]#(x), Cons(#1, #2)) →  Cons(#(#1), Map([x]#(x), #2));
...
\begin{figure}[h!t]
  ...
  \end{align}
  \vspace*{-2em}
  \caption{\hax syntax, Java Style.}
\end{figure}

\KR{Note: any suggestions for something other than \{:\#1\} are welcome!\\
  Just for fun: here is ``bra-ket'' notation (from physics), also kind of cool (note empty $⟨|$ and
  $|⟩$ can be omitted)…I'd love this, and it would be different and make us associate even more to
  the \hax symbol. The explanation then associates a \emph{bra} with a binder state and a \emph{ket} with
...
\end{hacs}
\KR{Modified syntax in Figure~\ref{fig:bra-ket} below.}
\begin{figure}[h!t]
  \begin{align}
    \tag{\hax{}Script}
    H &::= D^* 
  ...
  \end{align}
  \vspace*{-2em}
  \caption{\hax syntax, Bra-Ket Style.}
  \label{fig:bra-ket}
\end{figure}
\KR{P.S. The notation can be done with ⟨⟩ and [] reversed.}
...
\begin{example}[Peano]\label{ex:peano}
  ...
  \item The defined sort is "N". All the other declarations define artifacts of the "N" sort, so are
    inside the \kw{sort} declaration's \kw{\{\}}s.
...
\KR{Indeed! I think Lionel has it implemented as ``add''…but here we should describe the ideal as
  long as it allows for add, I thiink.}
\end{example}
...
\begin{figure*}[p]
  ...
  \begin{align}
    ...
    %
    &\dfrac
    { Γ,Δ,\op{Pat} ⊢ f(\ov{F}) : S  \qquad Γ,Δ,\op{Con} ⊢ T : S }
    { Γ ⊢ S~\kw{rule}~f(\ov{F})~\kw{→}~T\,\kw; }
    && (∃Δ),~f∈Γ_{\op{fun}}
    \tag{S-D-Rule}
    %
    \\
    \intertext{\shoveright{Sorting of Term\hfil\smash{\fbox{$ Γ,Δ,RS ⊢ T : S $}}}}
    %
    &\dfrac
    { Γ,Δ,RS ⊢ P_1 : F_1 \quad\cdots\quad Γ,Δ,RS ⊢ P_n : F_n }
    { Γ,Δ,RS ⊢ c\,\kw(\,P_1\kw,…\kw,P_n\,\kw) : S }
    &&\Cases{
    ...
    %
    &\dfrac
    { Γ,Δ,\op{Pat} ⊢ v_1 : S_1 \quad\cdots\quad  Γ,Δ,\op{Pat} ⊢ v_n : S_n }
    { Γ,Δ,\op{Pat} ⊢ M\,\kw[\,v_1\kw,…\kw,v_n\,\kw] : S }
    ...
    %
    &\dfrac
    { Γ,Δ,\op{Con} ⊢ T_1 : S_1 \quad\cdots\quad  Γ,Δ,\op{Con} ⊢ T_n : S_n }
    { Γ,Δ,\op{Con} ⊢ M\,\kw[\,T_1\kw,…\kw,T_n\,\kw] : S }
    && Δ_{\op{meta}}(M) = [(S_1,…,S_n)⇒S]
    \tag{S-T-MetaC}
    %
    \\
...
\begin{definition}[identifiers]
  ...
  \item \emph{Variable identifiers}. A variable identifier is globally unique.

  \item \emph{Constructor identifiers}. A constructor identifier is globally unique. From the
    constructor identifier one can derive the kind and signature of the constructor:
    \begin{itemize}
    \item Is it a function or data constructor?
